...
class Gen2Electrolyte(object):
    ...
    def get_t0(self, C_Li: float | _ndarray, T: float) -> float | _ndarray:
        ...
        t0 = (A[0, 0] * T**2 + A[0, 1] * T - A[0, 2]) * C_Li**2 \
           + (A[1, 0] * T**2 - A[1, 1] * T + A[1, 2]) * C_Li \
           + (A[2, 0] * T**2 + A[2, 1] * T + A[2, 2])
    ...
    def get_kappa(self, C_Li: float | _ndarray, T: float) -> float | _ndarray:
        ...
        kappa = (  A[0, 0] * T**4 + A[0, 1] * T**3 + A[0, 2] * T**2
                 + A[0, 3] * T + A[0, 4]  ) * C_Li**1 \
              + (  A[1, 0] * T**4 + A[1, 1] * T**3 + A[1, 2] * T**2
                 + A[1, 3] * T + A[1, 4]  ) * C_Li**2 \
              + (  A[2, 0] * T**4 + A[2, 1] * T**3 + A[2, 2] * T**2
                 + A[2, 3] * T + A[2, 4]  ) * C_Li**3 \
              + (  A[3, 0] * T**4 + A[3, 1] * T**3 + A[3, 2] * T**2
                 + A[3, 3] * T + A[3, 4]  ) * C_Li**4
    ...
    def get_gamma(self, C_Li: float | _ndarray, T: float) -> float | _ndarray:
        ...
        gamma = 0.54000 * np.exp( 329 / T) * C_Li**2 \
